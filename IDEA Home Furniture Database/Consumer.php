...
<head>
    <meta charset="UTF-8">
    <title>IDEA Furniture Database Interface For Administrators</title>
    <style>
        table {
...
<body>
    <h1>IDEA Furniture Database For Consumers Interface</h1>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['deliveryEmployee_id'])) {
    ...
    ?>
    <?php
// Function to add product ID to session array
function addProductToHistory($product_id) {
    // Initialize session array if not set
    if (!isset($_SESSION['product_history'])) {
        $_SESSION['product_history'] = array();
    }
    // Add product ID to session array
    array_push($_SESSION['product_history'], $product_id);
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['product_id'])) {
    ...
    // Add product ID to session history
    addProductToHistory($product_id);

    // Establish database connection and perform search...
}

// Display previous product searches
if (isset($_SESSION['product_history'])) {
    echo "<h2>Previous Product Searches:</h2>";
    echo "<ul>";
    foreach ($_SESSION['product_history'] as $product) {
        echo "<li>$product</li>";
    }
    echo "</ul>";
}
?>
<?php
// Function to add purchase ID to session array
function addPurchaseToHistory($purchase_id) {
    // Initialize session array if not set
    if (!isset($_SESSION['purchase_history'])) {
        $_SESSION['purchase_history'] = array();
    }
    // Add purchase ID to session array
    array_push($_SESSION['purchase_history'], $purchase_id);
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['purchase_id'])) {
    ...
    // Add purchase ID to session history
    addPurchaseToHistory($purchase_id);

    // Establish database connection and perform search...
}

// Display previous purchase searches
if (isset($_SESSION['purchase_history'])) {
    echo "<h2>Previous Purchase Searches:</h2>";
    echo "<ul>";
    foreach ($_SESSION['purchase_history'] as $purchase) {
        echo "<li>$purchase</li>";
    }
    echo "</ul>";
}
